...
/**
 * Wires the `/api/tabs`, `/api/tree/:tab`, `/api/contents/:tab`, `/api/file/:tab`,
 * `/api/navigator/tree`, and `/api/navigator/sprint-status` routes together for a specific
 * resolved project, in the shape `startHttpServer` (src/server/http-server.ts) expects.
 * Used by both `src/cli.ts` and the integration tests, so tests exercise the exact same
 * wiring the CLI runs.
 */
export function createApiRequestHandler(
  ...
  return async (pathname: string, searchParams: URLSearchParams): Promise<RouteResponse | undefined> => {
    ...
    if (pathname === "/api/navigator/tree") {
      return getNavigatorTreeResponse(root, cache);
    }
