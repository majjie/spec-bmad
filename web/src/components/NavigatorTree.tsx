 ...
 * grouping projects/dates/non-conforming folders, and a "Sprint Status" root (FR-011).
 * The "PRD" root and each project node are structural only (FR-010) — every click is
 * reported up via `onItemSelected`, and it's the caller's job (NavigatorView) to decide
 * which itemIds actually change the detail pane.
 */
export default function NavigatorTree({
  ...
  if (!tree || (tree.prd === null && !tree.sprintStatusAvailable)) {
    return (
